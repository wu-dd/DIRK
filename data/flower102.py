...
from data.dataset_flower import Flowers102
import PIL.Image


def load_flower102(args):
    ...
    original_train = Flowers102(root='./data/FLOWER102',split='train',transform=test_transform,download=False)
    ...
    test_dataset = Flowers102(root='./data/FLOWER102', split='test', transform=test_transform)
    ...
    model.load_state_dict(torch.load(os.path.expanduser('./partial_models/weights/flower102.pt')))
    ...
    partial_training_dataset = FLOWER102_Partialize(original_train._image_files,original_train._labels, partialY_matrix.float(), ori_labels.float())
