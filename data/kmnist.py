...
def load_kmnist(args):
    ...
    original_train = dsets.KMNIST(root='./data/KMNIST', train=True, download=True, transform=transforms.ToTensor())
    ori_data, ori_labels = original_train.data, original_train.targets.long()
    
    test_dataset = dsets.KMNIST(root='./data/KMNIST', train=False, download=True, transform=test_transform)
    ...
    partialize_net.load_state_dict(torch.load(os.path.expanduser('./partial_models/weights/kmnist.pt'),map_location='cuda:0'))
